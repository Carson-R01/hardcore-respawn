...
Mojang's username → UUID lookup — Mojang's own API can't be called directly
from a browser).
...
- You select your world's save folder (or the `playerdata` /
  ...
  layout — vanilla Minecraft actually uses `playerdata/` with no `players`
  parent folder, so both are supported.
...
1. **Delete the two named files directly** from your real world's player-data
   folder — the safest option, and unaffected by anything that's happened in
   the world since you exported it.
2. **Replace the whole player-data folder** with `RESET/`'s copy — only safe
   if nobody has played (and generated new/updated player files) since you
   exported the world, since this fully overwrites the folder.
